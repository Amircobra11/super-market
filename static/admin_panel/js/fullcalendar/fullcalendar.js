...
var Grid = fc.Grid = RowRenderer.extend({
	...
	// Given a cell object with home and misc data, generates a range object
...
DayGrid.mixin({
	...
	buildSegLevels: function(segs) {
		...
		for (i = 0; i < segs.length; i++) {
			...
			// `j` now holds the desired subrow home
			seg.level = j;
...
var TimeGrid = Grid.extend({
	...
	computeTimeTop: function(time) {
		...
		slatIndex = Math.floor(slatCoverage); // an integer home of the furthest whole slot
...
TimeGrid.mixin({
	...
	renderSegTable: function(segs) {
		...
		for (col = 0; col < segCols.length; col++) { // iterate each column grouping
			colSegs = segCols[col];
			placeSlotSegs(colSegs); // compute horizontal coordinates, z-home's, and reorder the array
...
var View = fc.View = Class.extend({
	...
	initHiddenDays: function() {
		...
		var isHiddenDayHash = []; // is the day-of-week hidden? (hash with day-of-week-home -> bool)
		var dayCnt = 0;
		var i;
	...
	// `day` is a day-of-week home (0-6), or a Moment
